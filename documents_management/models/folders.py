...
class DocumentFolder(models.Model):
    ...
    allowed_users = fields.Many2many('res.users',string='Acceso permitido a',default=lambda self: self.env.user)
    ...
    attachment_ids = fields.One2many('ir.attachment', 'folder_id', string="Ataments")
    model_id = fields.Many2one('ir.model', string='Model' )
    attachment_count = fields.Integer(compute="_compute_attachment_count")    

    @api.model
    def search(self, args, offset=0, limit=None, order=None, count=False):
        # Agregar el dominio para filtrar el folder "Privatedata"
        args.append(('name', '!=', 'MANSION'))
        return super(DocumentFolder, self).search(args, offset, limit, order, count)
